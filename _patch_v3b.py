...
# ─── 3. JS: onPaisChange → también resetear uCargo ──────────────────────────
old = (
...
# ─── 4. JS: populateReportaA → filtro + fallback + nuevas funciones ──────────
old = (
...
# ─── 5. JS: saveUser → cargo sin .trim() (select no lo necesita) ────────────
old = b"  const cargo=document.getElementById('uCargo')?.value||'';\r\n"
new = b"  const cargo=document.getElementById('uCargo')?.value||'';\r\n"
# Already correct, skip
results['saveUser cargo'] = True

# ─── Reporte ─────────────────────────────────────────────────────────────────
sys.stdout.buffer.write(b'Results:\r\n')
for k, v in results.items():
    status = b'OK' if v else b'NOT FOUND'
...
if all_ok:
    ...
    sys.stdout.buffer.write(b'ABORTED - check failures above.\r\n')
