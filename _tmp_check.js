...
const AREA_COLORS={
  ...
  'Construcción y Desarrollo':'background:#f1f5f9;color:#475569',
...
function avatarBg(r){return {
  rh:'#dcfce7',director:'#ede9fe',regional:'#dbeafe',zona:'#fef3c7',sucursal:'#f1f5f9',
  dir_admin:'#fce7f3',gerente_admin:'#ede9fe',jefe_admin:'#dbeafe',supervisor_admin:'#dcfce7',coordinador_admin:'#f1f5f9'
}[r]||'#dbeafe';}
      ...
      await addDoc(collection(db, 'mail'), {
        ...
        message: {
          ...
          html: `<p>Hola,</p>
                 ...
                 <p><a href="${downloadUrl}" style="background:#0f172a;color:#fff;padding:8px 12px;text-decoration:none;border-radius:6px;display:inline-block;">Descargar PDF</a></p>
                 <p>Saludos.</p>`
        }
      });
      ...
      Object.entries(areas).sort((a,b) => a[0].localeCompare(b[0])).forEach(([areaName, users], aIdx) => {
        ...
        users.forEach(u => {
          ...
          const areaBadgeStyle = u.esRhGlobal
            ...
            : (AREA_COLORS[areaKey2]||'background:#f1f5f9;color:#475569');
...
function showToast(msg,duration=3500){
  ...
  if(!t){t=document.createElement('div');t.id='toastEl';
    t.style.cssText='position:fixed;bottom:24px;right:24px;background:#1e293b;color:#fff;padding:12px 20px;border-radius:12px;font-size:13px;font-weight:600;z-index:9999;box-shadow:0 8px 24px rgba(0,0,0,.25);transition:opacity .3s;pointer-events:none';
...
function renderUauCharts(p){
  ...
  container.innerHTML=allFields.map(k=>`
    <div class="chart-card" style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:16px">
...
function buildUauPrintDoc(p){
  ...
  document.getElementById('printArea').innerHTML=`<div class="print-doc">
    ...
    <div class="psec" style="background:#f59e0b;color:#1e293b">INDICADORES HISTÓRICOS</div>
    <div style="overflow-x:auto;margin-bottom:16px">
      <table style="width:100%;border-collapse:collapse;font-size:9px">
        <thead style="background:#1e293b;color:#fff">${indHeaders}</thead>
        <tbody>${indRows}</tbody>
      </table>
    </div>
    <div class="psec" style="background:#f59e0b;color:#1e293b">RESUMEN DE LA SESIÓN INICIAL</div>
    ...
    ${segsHtml?`<div class="psec" style="background:#f59e0b;color:#1e293b">SEGUIMIENTOS REGISTRADOS</div>${segsHtml}`:''}
